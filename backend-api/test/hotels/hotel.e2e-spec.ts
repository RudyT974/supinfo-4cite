...
import { decoded_admin, decoded_customer, hotelData, HotelData1 } from './data.mock';
...
describe('UserController (e2e)', () => {
  ...
  it('/hotels (GET) - No Auth', async () => {
    return await request(app.getHttpServer()).get('/hotels').expect(200);
  });
  ...
  it('/hotels PATCH - Hotel should not exist', async () => {
    const token = await GenerateToken(decoded_admin);
